...
public:
  ...
  // serializes and sends an application-level message via the lumenprotocol layer
  void send_message(const Message &message,
                    const udp::endpoint &recipient = udp::endpoint());

  // sets the callback function to be invoked when a message is successfully deserialized
  void set_message_callback(
      std::function<void(std::unique_ptr<Message>, const udp::endpoint &)>
          callback);

  // sends a message directly via udp, bypassing lumenprotocol framing (sends raw json)
...
private:
  // internal callback handler called by lumenprotocol upon receiving a framed packet payload
